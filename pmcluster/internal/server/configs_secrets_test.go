...
func TestSecretsAPI(t *testing.T) {
	...
	// List shows the secret without the payload.
	resp = doJSON(t, http.MethodGet, base, "tok", nil)
...
func TestConfigsAPI(t *testing.T) {
	...
	// Create (scope+kind defaults exercised via explicit values).
	...
	// Update → new hash + version history entry.
	...
	// Versions: original content is retained in history (newest first).
