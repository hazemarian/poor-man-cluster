...
Runs as a global service on every node. Nightly tarballs of all app data under `/var/stack/data`, named with the node ID, `backup_retention_days` retention (default 15, configurable via `cluster settings`), optional S3 upload.
...
- `pmcluster user create|list|remove` — issue and manage API tokens for additional users (RBAC roles: admin/operator/viewer; tokens print once, hashed at rest)
...
│   ├── migrations/                     # *.sql (0001..0018), applied lexicographically and tracked in schema_version
...
6. Generate random bootstrap passwords for Traefik / OpenObserve / the edge console, store encrypted in SQLite, mirror to Swarm secrets
7. Render the OTel + Traefik dynamic configs in-process and create them as Docker configs (Swarm replicates to every node)
8. Deploy the `infra`, `edge`, `observability`, `backup`, and (if SSO enabled) `sso` stacks via `docker stack deploy`
9. Wait for the platform services to become healthy
10. Snapshot the rendered configs into the store (with content hashes)
11. Persist the install state (domain, OpenObserve admin email, TLS mode)
...
every 15s. On promotion it restores the control-plane DB from the newest
`pmcluster-ctlplane-*.tar.gz` archive **only if** the local DB is missing or
older — so shared-storage (e.g. LINSTOR) setups are never clobbered.
...
In addition to the per-node app-volume agent, the backup stack runs a manager-only **control-plane agent** (`control-plane-backup`). Every night it archives `~/.pmcluster` itself — the SQLite DB (users, API keys, webhook secrets, credentials, revisions), the encryption key, TLS certs, and rendered configs — into `/var/stack/backup` under a `pmcluster-ctlplane-` prefix (30-day retention). This closes the "lost manager disk = full re-bootstrap" gap: app data and the control plane both ride along in the nightly archive.
